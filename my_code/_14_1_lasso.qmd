...
gdrive_dir_enaho <- "/Users/snakada/Library/CloudStorage/GoogleDrive-snakada@g.ecc.u-tokyo.ac.jp/マイドライブ/Peru_work/Peru_enaho"

# 結果保存先
save_path <- file.path(gdrive_dir, "output", "lasso_14")
...
df_food_org <- readRDS(
    file.path(gdrive_dir_enaho, "output","food_consumption_by_area_income.rds")
  ) %>% 
  ...
  mutate(
    inc_quintile = as.numeric(as.character(inc_quintile))
  ) 
...
df_org <- df_org %>%
  ...
  mutate(
    # as.integer()で論理値を0/1に変換
    nt_fooddummy_staple_legume = as.integer(
      nt_grains == 1 | nt_nuts == 1 | nt_root == 1
    ),
    nt_fooddummy_vegetable_fruit = as.integer(
      nt_frtveg == 1 | nt_vita == 1
    ),
    nt_fooddummy_dairy = as.integer(
      nt_dairy == 1 | nt_fed_milk == 1
    ), 
    nt_fooddummy_ASF = as.integer(
      nt_meatfish == 1 | nt_eggs == 1
    )
  ) %>%
  var_labels(
    nt_fooddummy_staple_legume = "Staple and Legume consumption dummy",
    nt_fooddummy_vegetable_fruit = "Vegetable and Fruit consumption dummy",
    nt_fooddummy_dairy = "Dairy consumption dummy",
    nt_fooddummy_ASF = "Animal Source Food consumption dummy"
  )
...
# LASSO前の前データを一旦保存
saveRDS(df_org, file.path(gdrive_dir, "output", "df_lasso_before_preprocess.rds"))
...
# 除外変数定義(まずは多重共線性対応の観点から食事摂取量を全て除外：食品群摂取ダミーのみ)
drop_cols_add <- c(names(df_org)[grep("_log1p_score", names(df_org))], 
                   names(df_org)[grep("_optimal", names(df_org))],
                   names(df_org)[grep("nt_fooddummy", names(df_org))],
                   names(df_org)[grep("_log1p$", names(df_org))]
                   )
