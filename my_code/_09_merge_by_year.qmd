...
diagnose_join <- function(df1, df2, by_vars, join_name = "") {
  ...
  # キーのユニーク数
  for (key in common_keys) {
    n_unique_df1 <- df1 %>% dplyr::select(!!key) %>% 
      dplyr::filter(!is.na(!!sym(key))) %>% 
      dplyr::distinct() %>% 
      nrow()
    n_unique_df2 <- df2 %>% dplyr::select(!!key) %>% 
      dplyr::filter(!is.na(!!sym(key))) %>% 
      dplyr::distinct() %>% 
      nrow()
  ...
  cat("\n【マッチング診断】\n")
  unmatched_left <- df1 %>%
    dplyr::anti_join(df2, by = common_keys)
  unmatched_right <- df2 %>%
    dplyr::anti_join(df1, by = common_keys)
  
  cat(sprintf("  左側でマッチしない行数: %d (%.1f%%)\n", 
              nrow(unmatched_left), 
              100 * nrow(unmatched_left) / nrow(df1)))
  cat(sprintf("  右側でマッチしない行数: %d (%.1f%%)\n", 
              nrow(unmatched_right), 
              100 * nrow(unmatched_right) / nrow(df2)))
  
  # マッチング率の計算
  matched_rows <- df1 %>%
    dplyr::semi_join(df2, by = common_keys) %>%
    nrow()
...
left_join_safe <- function(df1, df2, by, join_name = "", diagnose = FALSE) {
  ...
  # 結合の実行
  result <- dplyr::left_join(df1_normalized, df2_normalized, by = by,
  ...
  if (nrow(result) > nrow(df1) * 1.5) {
    warning(sprintf("結合後の行数が元のデータの1.5倍以上になっています: %s (元: %d, 結果: %d)", 
...
diagnose_duplicates <- function(df, by_vars, df_name = "") {
  ...
  # キーの組み合わせで重複をチェック
  key_combo <- df %>%
    dplyr::select(all_of(by_vars)) %>%
    dplyr::filter(complete.cases(.))
  
  n_total <- nrow(key_combo)
  n_unique <- key_combo %>% distinct() %>% nrow()
...
goAnalysis <- function(year, enable_diagnostics = FALSE){
  ...
  print("(1) Starting with BRdata-rh")
  ...
  # ★★★ 修正箇所：専用関数を使用 ★★★
  data_merged <- merge_hrdata_dm_safe(
    data_merged, 
    data_all[["HRdata-dm"]], 
    diagnose = enable_diagnostics
  )
  ...
  # WASHデータの重複を事前に解決
  data_all[["WASHdata-ws"]] <- resolve_wash_duplicates(data_all[["WASHdata-ws"]])
  ...
  if (year == "2005") {
    ...
    if (!"caseid" %in% names(hr_data)) {
      stop("caseid列がHRdata-dmに見つかりません")
    }
    
    # caseidの最初2要素からキーを作成してhhidとマッピング
    hr_hhid_clean <- hr_data %>%
      dplyr::select(any_of(c("caseid", "hhid"))) %>%
      dplyr::filter(!is.na(hhid)) %>%
      mutate(
        caseid_key = paste(
          map_chr(str_split(str_squish(caseid), " "), ~.x[1]),
          map_chr(str_split(str_squish(caseid), " "), ~.x[2]),
          sep = " "
        )
      ) %>%
      dplyr::select(caseid_key, hhid) %>%
      distinct()
    
    # data_mergedにhhidを追加
    data_merged <- data_merged %>%
      dplyr::select(-any_of("hhid")) %>%
      mutate(
        caseid_key = paste(
          map_chr(str_split(str_squish(caseid), " "), ~.x[1]),
          map_chr(str_split(str_squish(caseid), " "), ~.x[2]),
      ...
      left_join(hr_hhid_clean, by = "caseid_key", 
                relationship = "many-to-one") %>%
      dplyr::select(-caseid_key)
    
    # WASHデータとの結合
    data_merged <- left_join_safe(
      data_merged, 
      data_all[["WASHdata-ws"]] %>% dplyr::select(-any_of("hhid_clean")),
      by = c("hhid"),
      join_name = "WASHdata-ws (2005)",
      diagnose = enable_diagnostics
    )
  ...
  data_merged <- left_join_safe(
    data_merged, 
    data_all[["PRdata"]], 
    by = c("caseid", "hhid", "year"),
    join_name = "PRdata",
    diagnose = enable_diagnostics
  ) %>%
    dplyr::filter(age >= 6 & age <= 24) %>%
    dplyr::select(-ends_with(".y")) %>%
    rename_with(~str_remove(., "\\.x$"), ends_with(".x"))
