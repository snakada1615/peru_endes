...
MRdata <- MRdata %>%
  mutate(
    ...
    rc_hins_any = case_when(
      mv481a == 1 | mv481b == 1 | mv481c == 1 | mv481d == 1 |
        mv481e == 1 | mv481f == 1 | mv481g == 1 | mv481h == 1 | mv481x == 1 ~ 1,
      TRUE ~ 0
    ),
    ...
    rc_smk_freq = mv463aa,
    ciga = case_when((mv464a >= 1 & mv464a < 888) ~ mv464a, TRUE ~ 0),
    cigb = case_when((mv464b >= 1 & mv464b < 888) ~ mv464b, TRUE ~ 0),
    cigc = case_when((mv464c >= 1 & mv464c < 888) ~ mv464c, TRUE ~ 0),
    cigdaily = ciga + cigb + cigc,
    rc_cig_day = case_when(
      (rc_smk_freq == 1) & (cigdaily >= 1 & cigdaily <= 4) ~ 1,
      (rc_smk_freq == 1) & (cigdaily >= 5 & cigdaily <= 9) ~ 2,
      (rc_smk_freq == 1) & (cigdaily >= 10 & cigdaily <= 14) ~ 3,
      (rc_smk_freq == 1) & (cigdaily >= 15 & cigdaily <= 24) ~ 4,
    ...
    rc_tobc_snuffm = case_when(
      (mv464h >= 1) | (mv484h >= 1) ~ 1, TRUE ~ 0
    ),
    rc_tobc_snuffn = case_when(
      (mv464i >= 1) | (mv484i >= 1) ~ 1, TRUE ~ 0
    ),
    rc_tobc_chew = case_when(
      (mv464j >= 1) | (mv484j >= 1) ~ 1, TRUE ~ 0
    ),
    rc_tobv_betel = case_when(
      (mv464k >= 1) | (mv484k >= 1) ~ 1, TRUE ~ 0
    ),
    rc_tobc_osmkless = case_when(
      (mv464l >= 1) | (mv484l >= 1) ~ 1, TRUE ~ 0
    ),
    rc_tobc_anysmkless = case_when(
      (mv464h >= 1) | (mv484h >= 1) | (mv464i >= 1) | (mv484i >= 1) |
        (mv464j >= 1) | (mv484j >= 1) | (mv464k >= 1) | (mv484k >= 1) |
        (mv464l >= 1) | (mv484l >= 1) ~ 1, TRUE ~ 0
    ),
    rc_tobc_any = case_when(
      mv463aa == 1 | mv463aa == 2 | mv463ab == 1 | mv463ab == 2 ~ 1, TRUE ~ 0
    )
  ) %>%
  replace_with_na(replace = list(eduyr = 99, rc_intr_usefreq = 99)) %>%
  set_variable_labels(
    rc_edu = "Highest level of schooling attended or completed",
    rc_litr_cats = "Level of literacy",
    rc_litr = "Literate - higher than secondary or can read part or whole sentence",
    ...
    rc_empl_earn = "Type of earnings among those employed in the past 12 months",
    rc_empl_cont = "Continuity of employment among those employed in the past 12 months",
    rc_agri = "Agriculture employment (dummy)",
    rc_hins_ss = "Health insurance coverage - social security",
    rc_hins_empl = "Health insurance coverage - other employer-based insurance",
    rc_hins_comm = "Health insurance coverage - mutual health org. or community-based insurance",
    rc_hins_priv = "Health insurance coverage - privately purchased commercial insurance",
  ...
  set_value_labels(
    rc_litr_cats = c(
      "Higher than secondary education" = 0,
    ...
    rc_empl = c(
      "Not employed in last 12 months" = 0,
      "Not currently working but was employed in last 12 months" = 1,
      "Currently employed" = 2,
      "Don't know/missing" = 9
    ),
    rc_occup = c(
      "Professional" = 1, "Clerical" = 2, "Sales and services" = 3,
      "Skilled manual" = 4, "Unskilled manual" = 5, "Domestic service" = 6,
      "Agriculture" = 7, "Other" = 8, "Don't know/missing" = 9
    ),
    rc_agri = c("Non-Agriculture" = 0, "Agriculture" = 1),
    rc_hins_ss = c("No" = 0, "Yes" = 1),
    rc_hins_empl = c("No" = 0, "Yes" = 1),
    rc_hins_comm = c("No" = 0, "Yes" = 1),
    rc_hins_priv = c("No" = 0, "Yes" = 1),
    rc_hins_other = c("No" = 0, "Yes" = 1),
    rc_hins_any = c("No" = 0, "Yes" = 1),
    rc_tobc_cig = c("No" = 0, "Yes" = 1),
    rc_tobc_other = c("No" = 0, "Yes" = 1),
    rc_tobc_smk_any = c("No" = 0, "Yes" = 1),
    rc_smk_freq = c("Non-smoker" = 0, "Daily smoker" = 1, "Occasional smoker" = 2),
    rc_cig_day = c("<5" = 1, "5-9" = 2, "10-14" = 3, "15-24" = 4, "25+" = 5, "Don't know/missing" = 9),
    rc_tobc_snuffm = c("No" = 0, "Yes" = 1),
    rc_tobc_snuffn = c("No" = 0, "Yes" = 1),
    rc_tobc_chew = c("No" = 0, "Yes" = 1),
    rc_tobv_betel = c("No" = 0, "Yes" = 1),
    rc_tobc_osmkless = c("No" = 0, "Yes" = 1),
    rc_tobc_anysmkless = c("No" = 0, "Yes" = 1),
    rc_tobc_any = c("No" = 0, "Yes" = 1)
  )

# インターネット利用関連（mv171a列の有無で分岐）の変数追加は if で条件分岐し、必要ならパイプチェーン内に改めて追加します。
if (!is.null(MRdata$mv171a)) {
  MRdata <- MRdata %>%
    mutate(
      rc_intr_ever = case_when(
        mv171a == 1 | mv171a == 2 | mv171a == 3 ~ 1,
        mv171a == 0 ~ 0
      ),
      rc_intr_use12mo = case_when(
        mv171a == 1 ~ 1,
        mv171a == 0 | mv171a == 2 | mv171a == 3 ~ 0
      ),
      rc_intr_usefreq = case_when(
        mv171a == 1 ~ mv171b,
        TRUE ~ 99
      )
    ) %>%
    replace_with_na(replace = list(rc_intr_usefreq = 99)) %>%
    set_variable_labels(
      rc_intr_ever = "Ever used the internet",
      rc_intr_use12mo = "Used the internet in the past 12 months",
      rc_intr_usefreq = "Internet use frequency in the past month - among users in the past 12 months"
