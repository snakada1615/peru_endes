...
goAnalysis <- function(year){
  ...
  keep_REC0111 <- c("caseid", "v161", "v005")
  ...
  keep_child_cooking_fuel <- c("caseid", "hhid", "v005", "dm_cooking_fuel", 
  ...
  df_REC0111 <- open_endes_file(year, "REC0111.sav") %>%
    rename_with(~ tolower(.x)) %>% # 変数名を小文字に変換
    mutate(
      hhid = substr(caseid, 1, 12), # 世帯IDをcaseidから抽出
    ) %>%
  ...
  IRdata <- IRdata %>%
    left_join(df_REC0111, by = c("hhid" = "hhid", "hv005" = "v005"))
  IRdata <- IRdata %>%
    left_join(cooking_fuel, by = c("hhid" = "hhid", "caseid" = "caseid"))
  #------------------------------------------------------------------------------------------------
