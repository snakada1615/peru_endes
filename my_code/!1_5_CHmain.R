...
goAnalysis <- function(year){
  ...
  # KRdata, KRvac, KRstoolにyear変数を追加
  KRdata <- KRdata %>% mutate(year = year)
  KRstool <- KRstool %>% mutate(year = year)
  KRvac <- KRvac %>% mutate(year = year)
  
  # KRdataから必要な変数を選択
  vars_to_keep <- KRdata %>%
    select(starts_with("ch")) %>%
    names() %>%
    append(c("year", "age", "v001", "v002", "v003", "v005", "v013", "v022", "v025", "v026", "v008", "b3", "b9", "caseid", "bidx")) # add the variables we need to keep
  ...
  write_dta(KRvac, paste0(outputDir, "/KRvac-ch.dta"))
  write_dta(KRstool, paste0(outputDir, "/KRstool-ch.dta"))
}
