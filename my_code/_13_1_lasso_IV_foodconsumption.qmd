...
saveRDS(df_lasso, file.path(save_path, "df_lasso.rds"))
...
## 4. LASSO選択変数を用いた2SLS分析（survey-weighted）
```{r}
library(survey)
...
# 予測値を元のデータフレームに保存
pred_dairy_raw <- predict(fit1_svy, type = "response")

# *** 重要：予測値を標準化・スケーリング ***
# 1st stageの標準化係数で使用したスケーリング情報を取得
pred_dairy_mean <- mean(pred_dairy_raw, na.rm = TRUE)
pred_dairy_sd <- sd(pred_dairy_raw, na.rm = TRUE)

# スケーリング（重要な処理）
df_lasso$pred_dairy <- (pred_dairy_raw - pred_dairy_mean) / pred_dairy_sd

# 統計量確認
cat("=== 1st Stage予測値の統計量 ===\n")
cat("生値（raw）:\n")
print(summary(pred_dairy_raw))
cat("\n標準化値:\n")
print(summary(df_lasso$pred_dairy))
cat("標準偏差:\n")
print(sd(df_lasso$pred_dairy, na.rm = TRUE))

# F統計量（弱操作変数検定）
f_stat <- summary(fit1_svy)$coefficients["juntos_non_participation", "t value"]^2
print(paste("F統計量（juntos_non_participation）:", round(f_stat, 2)))
if(f_stat > 10) {
...
formula_2nd <- as.formula(
  paste("nt_ch_stunt ~ ", paste(c("pred_dairy", selected_predictors), 
                    collapse = " + "))
)
...
# pred_dairyの係数と信頼区間
coef_pred_dairy <- coef(fit2_svy)["pred_dairy"]
ci_pred_dairy <- confint(fit2_svy)["pred_dairy", ]

print("=== 乳製品摂取のStuntingへの因果効果 ===")
print(paste("係数（標準化スケール）:", round(coef_pred_dairy, 4)))
print(paste("95%CI: [", round(ci_pred_dairy[1], 4), ",", round(ci_pred_dairy[2], 4), "]"))

# オッズ比に変換（標準化スケール）
or_pred_dairy <- exp(coef_pred_dairy)
or_ci <- exp(ci_pred_dairy)
print(paste("オッズ比（SD増加当たり）:", round(or_pred_dairy, 4)))
print(paste("95%CI: [", round(or_ci[1], 4), ",", round(or_ci[2], 4), "]"))

# 元のスケールでの効果を計算
coef_original_scale <- coef_pred_dairy / pred_dairy_sd
or_original_scale <- exp(coef_original_scale)

print("=== 元のスケール（log1p dairy consumption）での効果 ===")
print(paste("係数（元スケール）:", round(coef_original_scale, 6)))
print(paste("オッズ比（元スケール、1単位増加）:", round(or_original_scale, 6)))
print(paste("オッズ比（元スケール、10%増加）:", round(exp(coef_original_scale * log(1.1)), 4)))
