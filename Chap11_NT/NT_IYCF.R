...
KRiycf <- KRiycf %>%
  ...
  # 2005-2012のデータにはv412c/v414v/v414wが存在しないので修正
  KRiycf <- KRiycf %>%
    mutate(
      water  = case_when(v409 == 1 ~ 1, v409 != 1 ~ 0),
      liquids = case_when(
        v409a == 1 | v410 == 1 | v410a == 1 | v413 == 1 |
          v413a == 1 | v413b == 1 | v413c == 1 | v413d == 1 ~ 1,
        TRUE ~ 0
      ),
      milk = case_when(
        v411 == 1 | v411a == 1 ~ 1,
      ...
      solids = case_when(
        v414a == 1 | v414b == 1 | v414c == 1 | v414d == 1 | v414e == 1 |
          v414f == 1 | v414g == 1 | v414h == 1 | v414i == 1 | v414j == 1 |
          v414k == 1 | v414l == 1 | v414m == 1 | v414n == 1 | v414o == 1 |
          v414p == 1 | v414q == 1 | v414r == 1 | v414s == 1 | v414t == 1 |
          v414u == 1 | v412a == 1 | v412b == 1 |
          m39 > 1 ~ 1,
        TRUE ~ 0
      ),
      nt_bf_status = case_when(
        nt_bf_curr == 0 ~ 0,
        solids == 1 ~ 5,
        milk == 1 ~ 4,
        liquids == 1 ~ 3,
    ...
    set_value_labels(nt_bf_status = c(
      "not bf" = 0, "exclusively bf" = 1, "bf & plain water" = 2,
      "bf & non-milk liquids" = 3, "bf & other milk" = 4, "bf & complemenatry foods" = 5
    )) %>%
    set_variable_labels(
      nt_bf_status = "Breastfeeding status for last-born child under 2 years"
    )
...
KRiycf <- KRiycf %>%
  ...
  mutate(nt_liquids= case_when(v410==1 | v413==1  ~ 1 , v410!=1 | v413!=1  ~ 0)) %>% # Given other liquids
  mutate(nt_bbyfood  = case_when(v412a==1  ~ 1 , v412a!=1~ 0, TRUE ~ 0)) %>% # Given fortified baby food
  ...
  mutate(nt_dairy  = case_when(v414p==1 ~ 1 , v414p!=1 ~ 0)) %>% # Given dairy
