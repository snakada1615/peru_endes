...
# //Individual haz
PRdata <- PRdata %>%
  mutate(nt_ch_haz = haz) %>%
  set_variable_labels(nt_ch_haz = "Height-for-age z-score (haz) for children under 5 years")
...
# //Individual whz
PRdata <- PRdata %>%
  mutate(nt_ch_whz = whz) %>%
  set_variable_labels(nt_ch_whz = "Weight-for-height z-score (whz) for children under 5 years")
