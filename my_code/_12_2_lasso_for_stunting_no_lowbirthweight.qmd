...
df_lasso_df <- as.data.frame(df_lasso)
...
for (mediator in mediator_candidates) {
  if (mediator %in% names(df_lasso_df)) {
    
    # 共変量（メディエーター以外の選択変数）
    covariates_list <- setdiff(selected_predictors, mediator_candidates)
    covariates_formula <- paste(covariates_list, collapse = " + ")
    
    cat("=== メディエーター:", mediator, "の分析 ===\n")
    
    # 欠損値を除外したサブセット作成
    analysis_vars <- c(mediator, treatment_var, outcome_var, covariates_list, "year")
    df_complete <- df_lasso_df[complete.cases(df_lasso_df[analysis_vars]), ]
    ...
    mediator_formula_text <- paste(mediator, "~", treatment_var, "+", covariates_formula, "+ as.factor(year)")
    ...
    outcome_formula_text <- paste(outcome_var, "~", treatment_var, "+", mediator, "+", 
                                 covariates_formula, "+ as.factor(year)")
    ...
    tryCatch({
      med_out <- mediate(
        ...
        treat = treatment_var, 
        mediator = mediator,
        data = df_complete,  # 明示的にデータを指定
        control.value = 0,
        treat.value = 1,
        boot = FALSE,        # まずはブートストラップなしで試行
        sims = 1000         # quasi-Bayesian simulation
      )
...
if (length(mediation_results) > 0) {
  ...
  # 最初の成功したメディエーターでブートストラップを試行
  first_mediator <- names(mediation_results)[1]
  cat("ブートストラップ試行対象:", first_mediator, "\n")
  
  # 同じデータとモデルでブートストラップ版
  tryCatch({
    # 再度モデル構築（簡単のため）
    analysis_vars <- c(first_mediator, treatment_var, outcome_var, 
                      setdiff(selected_predictors, mediator_candidates), "year")
    df_boot <- df_lasso_df[complete.cases(df_lasso_df[analysis_vars]), ]
    
    mediator_formula <- as.formula(paste(first_mediator, "~", treatment_var, "+", 
                                        paste(setdiff(selected_predictors, mediator_candidates), collapse = " + "),
                                        "+ as.factor(year)"))
    outcome_formula <- as.formula(paste(outcome_var, "~", treatment_var, "+", first_mediator, "+",
                                       paste(setdiff(selected_predictors, mediator_candidates), collapse = " + "),
                                       "+ as.factor(year)"))
    
    model_M_boot <- glm(mediator_formula, data = df_boot, family = binomial)
    model_Y_boot <- glm(outcome_formula, data = df_boot, family = binomial)
    
    med_boot <- mediate(
      model_M_boot, model_Y_boot,
      treat = treatment_var, 
      mediator = first_mediator,
      data = df_boot,
      control.value = 0,
      treat.value = 1,
      boot = TRUE,
