...
if ("m17a" %in% names(BRdata)) {
  ...
  BRdata <- BRdata %>%
    mutate(rh_del_cestime = 99)  # m17aが無い場合は全部99に
}

BRdata <- BRdata %>%
  set_value_labels(rh_del_cestime = c(
    "Vaginal birth"       = 0,
    "before labor started" = 1,
    "after labor started"  = 2,
    "Not applicable"       = 99
  )) %>%
  set_variable_labels(rh_del_cestime = "Timing of decision to have Caesarean") %>%
  replace_with_na(replace = list(rh_del_cestime = c(99)))  # ラベルを残してNAに変換
