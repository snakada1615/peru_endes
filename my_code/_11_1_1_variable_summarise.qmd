...
# 分析に利用する変数名の抽出
variable_for_analysis <- read_excel(
...
for (i in seq_along(variables_list)) {
  ...
  tryCatch({
    ...
    cat((sprintf("Data loaded. %d observations, %d NAs. \n", nrow(df_single), sum(is.na(df_single$value)))))
    ...
    cat("Survey design created. Calculating means... \n")
    ...
    results_by_state[[i]] <- svy_obj %>%
      ...
      mutate(variable = var)
    cat("Means calculated. Now calculating means by treatment group... \n")
    ...
    results_by_trtGroup[[i]] <- svy_obj %>%
      ...
      mutate(variable = var)
    cat("Means calculated. Now calculating national means... \n")
    ...
    cat(sprintf("ERROR: %s\n", e$message))
    print(e)                 # 追加
    traceback()              # 追加（ただしコンソールでうるさければ一時的に）
    skipped_vars <<- c(skipped_vars, var)
  })
...
summary_by_state    <- bind_rows(results_by_state) %>% convert_lbl_to_paper() %>%
  rename_with(~ gsub("mean_value", "value", .x))
