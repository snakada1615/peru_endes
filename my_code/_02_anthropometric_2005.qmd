...
df_REC6_clean <- df_REC6 %>%
  filter(
    !is.na(hc1), !is.na(hc3), !is.na(hc27),
    hc1 >= 0 & hc1 < 60,           
    hc3 >= 450 & hc3 <= 1200,      
    hc27 %in% c(1, 2)              
  ) %>%
...
# WHOガイドラインに基づく異常値除外（±6 SD以上）
df_REC6_final <- df_REC6_clean %>%
  filter(hfaz >= -6 & hfaz <= 6)
...
# 平均HAZの95%信頼区間（単純）
mean_haz <- mean(df_REC6_final$hfaz, na.rm = TRUE)
se_haz <- sd(df_REC6_final$hfaz, na.rm = TRUE) / sqrt(nrow(df_REC6_final))
ci_lower <- mean_haz - 1.96 * se_haz
ci_upper <- mean_haz + 1.96 * se_haz

cat("\n平均HAZ:", round(mean_haz, 3), 
    " (95% CI: [", round(ci_lower, 3), ",", round(ci_upper, 3), "])\n")

# 分布の可視化
hist(df_REC6_final$hfaz, 
     breaks = 50,
     main = "HAZ分布（WHO 2006基準）",
     xlab = "Height-for-Age Z-score",
     ylab = "度数",
     col = "lightblue")
abline(v = -2, col = "red", lwd = 2, lty = 2)  # Stuntingカットオフ
abline(v = -3, col = "darkred", lwd = 2, lty = 2)  # 重度Stuntingカットオフ
legend("topright", 
       legend = c("HAZ < -2 (Stunting)", "HAZ < -3 (重度)"),
       col = c("red", "darkred"),
       lty = 2, lwd = 2)
