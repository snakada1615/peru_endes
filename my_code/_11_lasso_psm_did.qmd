...
# 実際の年度に基づくダミー変数作成
df_post$year2007 <- as.numeric(df_post$year == 2007)
df_post$year2008 <- as.numeric(df_post$year == 2008)
df_post$year2009 <- as.numeric(df_post$year == 2009)
df_post$year2010 <- as.numeric(df_post$year == 2010)
df_post$year2011 <- as.numeric(df_post$year == 2011)
df_post$year2012 <- as.numeric(df_post$year == 2012) 
df_post$year2013 <- as.numeric(df_post$year == 2013)
df_post$year2014 <- as.numeric(df_post$year == 2014)
df_post$year2015 <- as.numeric(df_post$year == 2015)
df_post$year2016 <- as.numeric(df_post$year == 2016)
...
selected_vars_post <- intersect(selected_vars, colnames(post_matrix))
...
missing_vars_post <- setdiff(selected_vars, colnames(post_covariates))
if(length(missing_vars_post) > 0) {
  for(var in missing_vars_post) {
    post_covariates[[var]] <- 0  # or NA だとエラー、必ず0
  }
  # 列順を selected_vars に合わせる
  post_covariates <- post_covariates[, selected_vars, drop=FALSE]
}
# ************************************************************************
...
# 適切なクラスター変数を選択（例：caseid, region, strata）
...
# 基本DIDモデル
did_model <- feols(nt_ch_stunt ~ treatment_group * post + 
                   factor(year),              # 年度固定効果
                   cluster = "cluster_var",   # クラスター調整
                   weights = final_weight,    # 重み適用
                   data = did_data)
...
```{r}
  ...
  if("w_combined_trimmed" %in% names(df_post)) {
    did_weights <- df_post %>% 
      select(caseid, year, w_combined_capped) %>%
      mutate(T = as.integer(year))
    
    did_data <- did_data %>%
      left_join(did_weights, by = c("caseid", "T")) %>%
      mutate(weights = ifelse(is.na(w_combined_capped), 1, w_combined_capped))
  } else {
    did_data$weights <- 1
  }
