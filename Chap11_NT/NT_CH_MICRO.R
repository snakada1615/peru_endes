...
if ("h80b" %in% names(KRdata)) {
  ...
  KRdata <- KRdata %>%
    mutate(nt_ch_food_ther = NA_real_) %>% # 暫定的に99を代入（set_value_labels()を通すため）
    set_value_labels(nt_ch_food_ther = c("Yes" = 1, "No" = 0)) %>%
    set_variable_labels(nt_ch_food_ther = "Children age 6-35 mos given therapeutic food (var missing)")
}

# //Received supplemental food
if ("h80c" %in% names(KRdata)) {
  KRdata <- KRdata %>%
  ...
  KRdata <- KRdata %>%
    mutate(nt_ch_food_supp = NA_real_) %>%
  set_value_labels(nt_ch_food_supp = c("Yes" = 1, "No" = 0)) %>%
    set_variable_labels(nt_ch_food_supp = "Children age 6-35 mos given supplemental food (var missing)")
}
