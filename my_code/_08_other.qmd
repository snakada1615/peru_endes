...
goAnalysis <- function(year){
  ...
  keep_REC0111 <- c("caseid", "v161", "hhid", "v005")
  ...
  df_REC0111 <- open_endes_file(year, "REC0111.sav") %>%
    rename_with(~ tolower(.x)) %>% # 変数名を小文字に変換
    mutate(
      # ゼロ埋めして文字列化（例：3桁 for v001, 4桁 for v002）
      v001_str = sprintf("%03d", v001),
      v002_str = sprintf("%04d", v002),
      hhid = paste0(v001_str, v002_str)
    ) %>%
  ...
  child_count <- df_RECH1 %>%
    ...
    select(hhid, dm_children_under12) %>%
    ungroup()
  ...
  cooking_fuel <- df_REC0111 %>%
    ...
    select(caseid, v005, dm_cooking_fuel, dm_cooking_fuel_traditional)
  print("調理燃料の分布を確認")
  ...
  IRdata <- child_count %>%
    ...
    left_join(cooking_fuel, by = c("caseid")) %>%
  ...
  print(names(df_RECH0))
  print(names(df_REC0111))
  print(names(df_RE516171))
  print(names(df_RECH23))
